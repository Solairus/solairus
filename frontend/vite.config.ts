...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      port: 8080,
      strictPort: true,
      // In Docker (start-dev.sh), fs events don't reliably cross the macOS↔Linux
      // bind mount, so poll for changes. No effect on native dev — DOCKER_DEV is
      // only set by docker-compose.dev.yml.
