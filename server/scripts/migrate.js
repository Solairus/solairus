...
async function run() {
  ...
  // Bootstrap: if schema_migrations is empty but DB already has prior schema,
  // mark older migrations (<= 004_) as applied to avoid re-running incompatible files.
  ...
  if ((migCount.rows[0]?.c ?? 0) === 0) {
    ...
    if (baseline.length) {
      console.log(`[migrate] bootstrapping schema_migrations with: ${baseline.join(', ')}`)
